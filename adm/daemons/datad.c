// 数据漫游 V-1.101 by Wenwu On 2007.09.16
...
// 数据库服务器
#define DATABASE_IP "xmud.kmip.net"
// 放置各站数据的目录
...
// 需要发送玩家资料档案的单个文件所在目录
nosave private string *user_file = ({ "login", "user" });
// 需要发送玩家资料档案批量文件所在目录
nosave private string *user_dir = ({ "item" });
// 分站的 IP 地址
nosave private string *agent_ip = ({ "211.99.3.95", "211.99.3.96" });
// 分站的 域名
nosave private string *agent_domain = ({ "www.migobt.com", "xmud.kmip.net", "www.zhaoshu.net" });

//___________________提外部调用的函数_____________________
void connect_to(object ob, string arg);
void send_database(object ob);
...
protected void parse_domain();
protected void parse_domain_callback(string address, string resolved, int key);
...
protected void in_listen_callback(int fd)
{
	...
	if ( !stringp(address) 
		|| sscanf(address, "%s %d", ip, port) != 2 
		|| member_array(ip, agent_ip) == -1 )
	{ // 未经授权的地址
		sys_msg(sprintf("站点 "HIR"%s"HIW" 地址没有经过授权", stringp(address) ? address : "unknow"));
		log_msg(sprintf("站点 %s 地址没有经过授权", stringp(address) ? address : "unknow"));
		in_close_callback(new_fd);
		return;
	}
...
protected void in_read_callback(int fd, mixed mess)
{
	...
	{ // 验证
		...
		if ( file_name[(name_size-2)..(name_size-1)] == ".b" )
			load_object(replace_string(file_name[0..<3], "/binaries/", "/"));*/
	...
	if ( sv_info[fd][VALID] )
	{
		...
		if ( sv_info[fd][FILE_NAME] )
		{
			...
			{ // 发送完成
				if ( sv_info[fd][FILE_SIZE] > MAX_SHOW_RATE )
					sys_msg(BBLU+HIR"[ "HIG+fd+HIR" ]"NOR+HIW" 接收进度 "HIY+sv_info[fd][FILE_POS]+" 字节 "HIW"("HIG+(sv_info[fd][FILE_POS]*10/(sv_info[fd][FILE_SIZE]/10))+"%"HIW")");
				sys_msg(BBLU+HIR"[ "HIG+fd+HIR" ]"NOR+HIW" 接收到文件 "HIG+sv_info[fd][FILE_NAME]+HIW" ("HIY+sv_info[fd][FILE_SIZE]+"字节"HIW")");
...
protected void sys_msg(string msg)
{
	message("wiz", HIR"数据服务："HIW+msg+"\n"NOR, users());
}

// 记录
protected void log_msg(string msg)
{
	log_file("/net/datad", sprintf("%s：%s\n", CHINESE_D->chinese_time(7, time()), msg));
}
...
protected void send_read_callback(int fd, mixed mess)
{
...
#endif
	msg = replace_string(msg, DATABASE_DIR+"/", "/");
	...
	if ( sscanf(msg, REPLY_MESSAGE, type, filename) == 2 )
	{
		switch ( type )
		{
			// 开始发送文件
			case VALID_SEND :
				filename = replace_string(filename, "/"+CHINESE_MUD_NAME+"/", "/");
			...
			case FINISH :
				...
				if ( cv_info[fd][FINISH_FILE] <= cv_info[fd][MAX_FILE] )
				{
					...
					{ // 全部发送完成，发送一条信息通知
						...
						if ( sizeof(max=cv_info[fd][WAIT_FILE]) && filename != "reset" ) 
						{
...
{
	mapping party;
	string id, send, party_id;
	int i;

	if ( !objectp(ob) )
	{ // 无效的obj
		sys_msg(HIY"传入了无效的 "HIG"OBJECT"HIY" 参数");
		return;
	}
	if ( !stringp(id = ob->query("id")) )
	{ // 没有获取正确的 ID
		sys_msg(HIY"无法获取 "HIG"ID"HIY" 标识");
		return;
	}
	for ( i=0; i<sizeof(user_file); i++ )
	{
		send = sprintf("%s %d,%s/%s/%s.o,%s", DATABASE_IP, SV_PORT, 
			DATA_DIR+user_file[i], id[0..0], id, SERVER_PASS);
		connect_to(0, send);
	}
	for ( i=0; i<sizeof(user_dir); i++ )
	{
		send = sprintf("%s %d,%s/%s/,%s", DATABASE_IP, SV_PORT, 
			DATA_DIR+user_dir[i], id, SERVER_PASS);
		connect_to(0, send);
	}
	//_____________________________发送帮会资料____________________________
	if ( !mapp(party = ob->query("party")) ) return;
	if ( !stringp(party_id = party["id"]) ) return;
	if ( !party["zhu"] && !party["zhanglao"] ) return;

	send = sprintf("%s %d,/d/party/%s/,%s", DATABASE_IP, SV_PORT, party_id, SERVER_PASS);
	connect_to(0, send);
}
...
protected void parse_domain()
{
	...
	for ( i=0; i<sizeof(agent_domain); i++ )
	{
		resolve(agent_domain[i], "parse_domain_callback");
	}
}
...
{
	if ( resolved 
		&& sscanf(resolved, "%*d.%*d.%*d.%*d") 
		&& member_array(resolved, agent_ip) == -1 )
		agent_ip += ({ resolved });
}
