// 修改完善，采用新的验证方式，支持配置文件配置相关参数 By Wenwu
...
int check_valid_read( string fname, int fd )
{
	...
	// 增加读写权限限制，只有高级巫师才可以读 目录(u) 以外的的区域
	if ( wizhood(UNAME) != "(boss)" && !sscanf(fname, HOME_DIR(UNAME[0..2])+"%*s") )
	{
		log_file( LOG_FILE, sprintf("%s read to %s.\n", UNAME, fname) );
		return 0;
	}
	/*
...
int check_valid_write( string fname, int fd )
{
	...
	if ( wizhood(UNAME) != "(boss)" && !sscanf(fname, HOME_DIR(UNAME[0..2])+"%*s") )
	{
		log_file( LOG_FILE, sprintf("%s write to %s.\n", UNAME, fname) );
		return 0;
	}
