...
void get_job()
{
	...
	if (robot->query("family/family_name") != target["party"])
	{
		message("channel:chat", HIC"\n【门派仇杀】"+robot->query("family/family_name") + ":众弟子听令!"NOR"。\n", users());
		message("channel:chat", HIC"\n【门派仇杀】"+robot->query("family/family_name") + ":顺我者生，逆我者亡!"NOR"。\n", users());
		message("channel:chat", HIC"\n【门派仇杀】"+robot->query("family/family_name") + ":" + target["party"]+"与我派做对，这一次誓必要消灭"+target["party"]+"!"NOR"。\n", users());
...
{
    message("channel:chat", HIC"\n【门派对抗】"/*+robot->query("family/family_name") */+ ":哼，这次就先放你们一马，下次一定杀得你们鸡犬不留！"NOR"。\n", users());
    destruct(robot);
    remove_call_out("get_job");
