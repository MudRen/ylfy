...
// 互联站点列表
mapping xyzx_mudlist;
mapping xyzx_dnslist;
// 检查启动状况，为 1 则启动完毕
int xyzx_dns_doing = 0;
// 本地主机名，支持域名
string local_host_name = "127.0.0.1";
...
void read_callback(int sock, string msg, string addr)
{
	...
	while( i-- ) {
		...
		if( bootsrv[0] == addr && bootsrv[1] == args["PORTUDP"] )
			if ( file_size(AUX_PATH + func + ".c") > 0 || file_size("/binaries" + AUX_PATH + func + ".b" ) > 0 )
			{
				flag = 1;
			}
	}
...
string start_message()
{
	return sprintf( "||MUDNAME:%s||NAME:%s||VERSION:%s||DRIVER:%s||MUDLIB:%s"
		"||HOST:%s||PORT:%d||PORTUDP:%d||TIME:%d||ENCODING:%s||USERS:%d||TCP:%s", 
		CHINESE_MUD_NAME,Mud_name(),MUDLIB_VERSION, __VERSION__, MUDLIB_NAME,
		query_host_name(),mud_port(), my_port, uptime(), MUDLIB_ENCODING,
		sizeof(get_all_user()), TCP_SERVICE_LEVEL);
}
...
mapping query_muds()
{
	...
	if ( local_host_name != "127.0.0.1" && sscanf(local_host_name, "%d.%d.%d.%d", ip1, ip2, ip3, ip4) != 4 )
		resolve(local_host_name, "resolve_callback");
	
	return muds + 
		([
			mud_nname():this_host + 
			([ "TIME":""+uptime() ]) +
			([ "USERS":sprintf("%d",sizeof(get_all_user())) ])
		]);
}
...
void start_dns_mudlist()
{
	string file_arg, *mudlist, mud_name, ip;
	int i, port, ip1, ip2, ip3, ip4;
	
	if ( file_size(CONFIG_DIR + "mudlist") > 0 )
	{
		file_arg = read_file(CONFIG_DIR + "mudlist");
		mudlist = explode(file_arg, "\n");
		xyzx_mudlist = allocate_mapping( sizeof(mudlist) );
		xyzx_dnslist = allocate_mapping( sizeof(mudlist) );
		for ( i = 0; i < sizeof(mudlist); i++ )
		{
			if ( sscanf(mudlist[i], "%s:%s %d", mud_name, ip, port) == 3 )
			{
				if ( !sscanf(ip, "%d.%d.%d.%d", ip1, ip2, ip3, ip4) )
				{
					xyzx_dnslist[mud_name] = ip + " " + port;
					resolve(ip, "resolve_ip_callback");
				}
				else
					xyzx_mudlist[mud_name] = ip + " " + port;

			}
			else if ( sscanf(mudlist[i], "#local_host_name:%s", mud_name) )
			{
				if ( mud_name != "" )
					local_host_name = mud_name;
			}
		}
	}
	else
	{
		xyzx_mudlist = ([]);
		xyzx_dnslist = (["ylfy":"xmud.kmip.net 2007"]);
	}

	start_mudlist_going();
	remove_call_out("address_to_ip");
	call_out("address_to_ip", 30, 0);
}
...
void address_to_ip()
{
	...
	if ( !mapp(xyzx_dnslist) )
		return;
	if ( !sizeof(xyzx_dnslist) )
		return;

	addr = keys(xyzx_dnslist);
	for ( i = 0; i < sizeof(addr); i++ )
	{
		if ( sscanf(xyzx_dnslist[addr[i]], "%s %d", ip, port) )
			resolve(ip, "resolve_ip_callback");
	}
...
void resolve_ip_callback(string address, string my_ip, int key)
{
	...
	mudlist_name = keys(xyzx_dnslist);
	...
	{
		for ( i = 0; i < sizeof(mudlist_name); i++ )
		{
			if ( sscanf(xyzx_dnslist[mudlist_name[i]], address + " %d", port) )
				xyzx_mudlist[mudlist_name[i]] = my_ip + " " + port;
		}
	}
}
...
/*#ifdef __SENSIBLE_MODIFIERS__
	my_port = SRVC_PORT_UDP(get_config(15)); // Add By Wenwu
#else
	my_port = SRVC_PORT_UDP(get_config(__MUD_PORT__)); // Add By Wenwu
#endif*/
	my_port = get_system_config(DATA_CONFIG_DIR "net.h", "MUDLIST_PORT");
	...
	this_host = ([
		...
		"HOSTADDRESS"	: "127.0.0.1",			// set in resolve_callback()
		"PORT"		: "" + (get_system_config(DATA_CONFIG_DIR "net.h", "MUDLIST_PORT")-4),

/*#ifdef __SENSIBLE_MODIFIERS__
		"PORT"		: "" + get_config(15), // Add By Wenwu
#else
		"PORT"		: "" + get_config(__MUD_PORT__), // Add By Wenwu
#endif*/

		"PORTUDP"	: "" + get_system_config(DATA_CONFIG_DIR "net.h", "MUDLIST_PORT"),
		"ADMIN"		: MUD_ADMIN,
		"HTTP"		: MUD_WEB,
	...
	if ( local_host_name && local_host_name != "127.0.0.1" )
	{
		if ( !sscanf(local_host_name, "%d.%d.%d.%d", ip1, ip2, ip3, ip4) )
			resolve(local_host_name, "resolve_callback");
		else
			this_host["HOSTADDRESS"] = local_host_name;
	}
	else
	...
	xyzx_dns_doing = 1;
	CHANNEL_D->do_channel(this_object(), "sys", "MUD 互联核心 开始与各同步站点互联。 \n");
}
