...
void create()
{
   ...
   maps=([]);
   file = read_file("/kungfu/dynamic_location");
   roomlines = explode(file,"\n");
   file = read_file("/kungfu/dynamic_quest");      
   quests = explode(file,"\n");
   file = read_file("/kungfu/find.map");
   buf1 = explode(file,"\n");
   size=sizeof(buf1);
   if(!size) return;
...
int quest_reward(object me, object who,object quest_item)
{
  ...
  tell_object(me, sprintf("Äã»ñµÃÁË"HIR"%s"NOR"µÄ¾­ÑéÖµ ,"HIY"%s"NOR"µãÇ±ÄÜ£¡\n"NOR, chinese_number(exp*me->query_exp_times()),chinese_number(pot)) );
CHANNEL_D->do_channel(this_object(),"rumor",sprintf(HIG+me->name()+NOR"½«Ñ°µ½µÄ"HIW+quest_item->name()+NOR"ËÍ¸ø"+who->name()+"»ñµÃ"HIR+chinese_number(exp*me->query_exp_times())+NOR"µÄ¾­Ñé£¬"HIY+chinese_number(pot)+NOR"µãÇ±ÄÜ\n"));
...
output =" [1;32m     ¡ù [31mÏ¦ÑôÔÙÏÖÊ¹Ãü°ñ [32m¡ù [m\n"+
                "©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥\n"+output+
                "©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥\n\n";
