...
#define MAX_TEAMS_PER_IP	0	//每个ip允许2个副本
...
/***************** 迷宫的一些预设特性：*****************/
private int l = MAX_LONG;				// 迷宫的单边长
private string *inherit_rooms = ({"/clone/map/map"});	// 迷宫允许继承的档案名称
private string entry_dir = "west";		// 迷宫入口方向
private string link_entry_dir = "west";		// 迷宫入口与区域的连接方向
private string link_entry_room = "/d/city/wumiao";		// 迷宫入口所连接区域档案的文件名
private string link_exit_dir = "east";		// 迷宫出口与区域的连接方向
private string link_exit_room = "/d/automap/"+random(get_system_config(DATA_CONFIG_DIR "auto_map.h", "MAX_SIZE"));		// 迷宫出口所连接区域档案的文件名
private string entry_short = "副本入口";		// 迷宫入口的短描述
private string entry_desc = "这里是副本的入口处，再往前面就是传说中的险恶之地寂静之森了，来\n" + 
							"到了这里就意味着再也没有退路，只有穿过前面的寂静之森才能找到逃生之\n" + 
							"路。\n";		// 迷宫入口的长描述
private string exit_short = "副本出口";		// 迷宫出口的短描述
private string exit_desc = "这里就是副本的出口处了，只有穿过寂静之森才能到达这里，古往今来\n" + 
							"又有多少英雄豪杰葬身其中，能来到这里的少之又少。前面不远处似乎是迷\n" + 
							"雾森林，又是一个新的冒险的开端。\n";		// 迷宫出口的长描述
private string *maze_room_desc = ({"这里就是传说中的寂静之森，树茂林密难见天日，四周静悄悄的充斥着\n" 
								 + "一片死寂。看似的幽静却不知吞没了多少英雄侠士，宁静的表面下正汹涌着\n" 
								 + "澎湃的杀机，真所谓的步步危机。\n"});	// 迷宫房间的长描述
private string maze_room_short = "寂静之森";		// 迷宫房间的短描述
private int is_outdoors = 1;		// 迷宫房间是否为户外
private string *maze_npcs = ({ // 迷宫中的怪物
	"/clone/migong/npc/guai1", 
	});
private string *new_npcs = ({ // 新手副本怪物
	"/clone/migong/npc/guai",
	});
/*_________________________________冥府_________________________*/
private mapping hades = ([
	"one" : ([
				"entry_short" : "冥府入口",
				"entry_long" : "这里就是冥府的入口处，再往前就是第一层地狱了，但觉到处阴森恐怖,\n"+
								"令人脊背阵阵发麻，汗毛直竖。\n",
				"exit_short" : "冥府一层出口",
				"exit_long" : "这里就是冥府一层出口处，前面直通地狱二层，但觉到处阴森恐怖,\n"+
								"令人脊背阵阵发凉，汗毛直竖。\n",
				"room_short" : "冥府一层",
				"room_long" : "这里就是地狱一层，时不时的传来阵阵鬼哭狼啼让人不寒而栗，隐隐约约\n"+
								"但见前面黑影栋栋，似乎隐藏着重重危机。\n",
...
{
	int random_rate = 30;	// 房间内放置 npc 的可能性 原本为30
	int idx,x,y,exits;
	object ob, *team_obs;
	string f, place;
	...
	// 新手副本
	//---------------------start--------------------
	if ( place == "new" ) random_rate = 90; // 刷新几率
	//----------------------end---------------------
	// 冥府
	//-------------------------------------------------
	if ( place == "hades" ) random_rate = 60; // 刷新几率
	//-------------------------------------------------
	...
	if ( place == "new" )
	{
		...
		{
			ob->set("objects",([
				new_npcs[random(sizeof(new_npcs))] : 1, //原1
				]));
			ob->setup();
		}
	}
	...
	{
		ob->set("objects",([
			maze_npcs[random(sizeof(maze_npcs))] : 1, //原1
			]));
		ob->setup();
	}
