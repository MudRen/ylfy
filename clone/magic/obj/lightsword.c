...
void create()
{
        ...
        if (clonep())
		      ...
		      //set("no_drop", 1);
                ...
                set("weapon_prop/strength", 20000);	
                set("weapon_prop/dexerity", 20000);
                set("weapon_prop/dodge", 20000);
                set("weapon_prop/parry", 20000);
                set("treasure",1);       
...
mixed hit_ob(object me, object victim, int damage_bonus)
   {
    ...
    if ( random(me->query("combat_exp"))>(int)victim->query("combat_exp")/5)
       {
    if (!victim->is_busy())
        victim->start_busy(5);
