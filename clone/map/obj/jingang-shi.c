...
void create()
{	      
	set_name( HIW"万年玄冰(假)"NOR, ({"jingang shi"}) );
    set_weight(5);
	set("long","这便是传说中的万年玄冰？你突然有种想把他熔炼(ronglian1)到30级以上动态装备上，来提升动态装备属性。\n"NOR);	
		set("unit", "块");
		set("value", 1);
		set("owner_id",1);
...
void init()
{
    if ( environment() == this_player() )
		add_action("do_ronglian1", "ronglian1");
}

int do_ronglian1(string arg)
{
	object me, ob, obj;
	...
	if ( !arg )
		return notify_fail("你要把万年玄冰(假)熔炼到什么上？\n");
	
	if ( sscanf(arg, "%s to %s", str, item) != 2 )
		return notify_fail("指令格式：ronglian1 材料英文名 to 装备英文名\n");
	...
	if ( !obj->query("armor_prop/armor") && !obj->query("weapon_prop/damage") )
		return notify_fail("你只能用万年玄冰(假)提升武器或防具的等级。\n");
    
	if ( obj->query("equipped") )
      return notify_fail("你必须解除对这件物品的装备才能用万年玄冰(假)熔炼。\n");
	...
	// 失败率设定
	if( random(100) < 90 ){
		message_vision(HIM"$N试图把万年玄冰(假)熔炼到" + obj->query("name") + HIM"上提升等级，万年玄冰(假)却莫名其妙地消失了。\n"NOR, me);
	...
	if ( obj->query("weapon_prop/damage") )
	{
		message_vision(HIM"$N把万年玄冰(假)熔炼到" + obj->query("name") + HIM"上，" + obj->query("name") + HIM"的等级提升至"HIW + chinese_number(obj->query("weapon_prop/level") + 1) + "级" + HIM"。\n"NOR, me);
	...
	{
		message_vision(HIM"$N把万年玄冰(假)熔炼到" + obj->query("name") + HIM"上，" + obj->query("name") + HIM"的等级提升至"HIW + chinese_number(obj->query("armor_prop/level") + 1) + "级" + HIM"。\n"NOR, me);
