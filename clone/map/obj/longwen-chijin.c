...
void init()
{
    if ( environment() == this_player() )
		add_action("do_ronglian2", "ronglian2");
}

int do_ronglian2(string arg)
{
	object me, ob, obj;
	...
	if ( !arg )
		return notify_fail("你要把龙纹赤金熔炼到什么上？\n");
	
	if ( sscanf(arg, "%s to %s", str, item) != 2 )
		return notify_fail("指令格式：tie 材料英文名 to 装备英文名\n");
	...
	if ( !obj->query("armor_prop/armor") && !obj->query("weapon_prop/damage") )
		return notify_fail("你只能用龙纹赤金提升武器或防具的等级。\n");
    
	if ( obj->query("equipped") )
      return notify_fail("你必须解除对这件物品的装备才能用龙纹赤金熔炼。\n");
	...
	if( random(100) < 80 ){
		if (random (100) > 50){
		message_vision(HIM"$N试图把龙纹赤金熔炼到" + obj->query("name") + HIM"上提升等级，龙纹赤金却莫名其妙地消失了。\n"NOR, me);
	...
	if ( obj->query("weapon_prop/damage") )
	{
		message_vision(HIM"$N把龙纹赤金熔炼到" + obj->query("name") + HIM"上，" + obj->query("name") + HIM"的等级提升至"HIW + chinese_number(obj->query("weapon_prop/level") + level) + "级" + HIM"。\n"NOR, me);
	...
	{
		message_vision(HIM"$N把龙纹赤金熔炼到" + obj->query("name") + HIM"上，" + obj->query("name") + HIM"的等级提升至"HIW + chinese_number(obj->query("armor_prop/level") + level) + "级" + HIM"。\n"NOR, me);
