...
void check(object me)
{
 ...
 /*   if( oldcrypt(me->query_temp("check"),yn)!=yn) {
        ...
        message("sendto_alluser",HIW"〖泥潭福彩〗："+HIC" "+me->query("name")+HIW" 中了"+HIY" "+chinese_number(money)+HIW" 两黄金，已经拨入帐户！\n"NOR,users());
        all_biao[me->query_temp("num")]["jieguo"]="已兑奖";
    ...
    if(!query("duijiang"))
    {
        ...
        while (i<h)
        {
            ...
            /*if (strlen(name["name"])==6)
                str+= sprintf("┃%-28s┃%22s┃%-6s┃\n",HIR+name["name"]+NOR,(int)name["z_num"],name["jieguo"],);//删除name["jieguo"]前(int)name["t_num"] 
...
int do_duijiang()//兑奖指令
{
    ...
    else if ( k )
    {
        ...
        me->set_temp("num",k-1);
        write("您的中奖号码是"+me->query_temp("check")+"。\n");
     //   write("请输入您的校验码:");
        //input_to("check",me);
		check(me);
        return 1;
...
void auto_post(string title,string body)
{
    ...
    note["title"] = title;
    note["author"] ="彩票板";
   // "/cmds/std/news.c"->done_post(this_object(),note,body);暂注释掉新闻提醒领奖
    return;
}
void clean_data()
{
