...
set_name(HIR"元旦贺卡"NOR, ({"new year card", "card"}));       	
        if( clonep() )
                ...
                set("unit", "张");
                set("long",
"这是一张"HIR"元旦贺卡"NOR"，送给每位在元旦节进入风云再起       	
...
{
        object newcard, box, obj0, obj1, obj2, obj3, obj4, obj5;
        object me = this_player();
        if (arg == "card")
        {
       message_vision("$N充满憧憬地拆开了元旦卡，突然如魔术般，出现	一个礼品盒！\n", me);
