...
int main(object me, string arg)
{
	...
	if( arg=="cancel" ) 
	{
		...
		if( !objectp(old_rec) ) return notify_fail("你现在并没有收录任何人为弟子的意思。\n");
	...
	//if(me->query("id") == "china") return notify_fail("模拟id禁止收徒.\n");	
	...
	if ( me->is_fighting() || ob->is_fighting() )
		return notify_fail("现在正打的起劲，不是行收徒仪式的时候。\n");
	if ( ob->name() != ob->name(1) )
		return notify_fail("收徒？你要收谁呀？\n");
	if( !me->query("family") ) return notify_fail("你并不属于任何门派，你必须先加入一个门派，或自己创一个才能收徒。\n");
	...
	if ( (object)ob->query_temp("pending/apprentice") == me ) 
	{
		...
		{
			/*message_vision("$N由于已拜有师尊，在未出师前无法改投到$n门下！！\n\n", ob, me);
			return 1;*/
			message_vision(
				"$N决定背叛师门，改投入$n门下！！\n\n"
