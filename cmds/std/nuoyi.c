...
int main(object me, string arg)
{
	...
	if( form == 1)
	{
		if( (int)me->query_temp("nuoyi") || me->query_temp("nuoyi_br") > time() || me->query_temp("powerup")) 
			return notify_fail("你已经在运功中了。\n");
		if( !me->is_fighting() )
			return notify_fail("乾坤大挪移只能在战斗中使用。\n");
		message_vision( HIR"$N默运乾坤大挪移大法，脸色忽青忽红，连变"CYN + lvl + HIR"次，将全身内力尽数提起！\n"NOR, me);
		me->add_temp("str", r_lvl);
		me->set_temp("nuoyi", 1);
		me->set_temp("powerup", 1);
		me->set_temp("nuoyi_br",time()+skill);
		me->start_call_out( (: call_other, __FILE__, "remove_effect", me, r_lvl :), skill);
		me->add("neili", -300);
		me->start_busy(3);
		return 1;
	}
