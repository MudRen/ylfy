...
int main(object me, string arg)
{
	...
	if ( me->query_skill("longxiang",1) < 150 && me->query_skill("xiaowuxiang",1) < 150 )
		return notify_fail("你的雪山内功修为不够无法行使摄心！\n");
	...
	if( !userp(ob) )
		return notify_fail("你只能对玩家施用慑心大法。\n");

	if( ob->query("age") < 18 && !ob->query_condition("killer") )
		return notify_fail("你不会连小孩都不放过吧？\n");

	if( me->is_busy() )
		return notify_fail("你上一个动作还没有完成，没法行使慑心术。\n");
	
	if( wiz_level(me) < wiz_level(ob) )
		return notify_fail("你没法对" + ob->name() + "行使慑心术。\n");
	
	message_vision("$N的一对眼睛半眯着，只如两道细缝，但细缝中莹然有光，眼神甚是清朗。$n微觉不妥，要想转头避开$N的眼光，可是一双眼睛竟似被$N的目光吸住了，不由自主的凝视着他。\n", me, ob);
