...
int main(object me, string arg)
{
	    ...
	    int n;
        //if(!wizardp(me) ) 
			//return notify_fail(HIY"想干什么?你疯了吗!\n"NOR);
        ...
        if(obj==me)     
			return notify_fail(HIW" 强奸自己? 没听说过！\n"NOR);
        ...
        if (userp(me) && userp(obj) && me->query_temp("nokill") )
            return notify_fail("你启用了免战牌！\n");

        if( userp(me) && userp(obj) && obj->query_condition("nokill")&& !obj->query_condition("pker"))
