...
int do_suan(string arg)
{
        ...
        switch(random(6)) {
                ...
                if( str1+ntg1+dex1+per1+con1+kar1 > (ob->query("age")+6) * 6) {
                        command("nod");
                        say("不错，"+ob->name()+"也还算修练有方。");
                } else {
                        command("sigh");
                        say(ob->name()+"还要在基本功上多下点功夫！");
                }
                break;
                case 1 :
                if(kar > 22) {
                        command("smile");
                        say(ob->name()+"福缘深厚，吉人自有天相。");
                }else {
                        command("hmm");
                        say(ob->name()+"这一生看来是注定要多历磨难了。");
                }
                break;
                case 2 :
                if(ntg > 25 || ntg1 > 40+ob->query("age") ) {
                        command("nod");
                        say(ob->name()+"天资过人，若能多加努力，必定能出人头地。");
                } else {
                        command("shake");
                        say(ob->name()+"如果知道＂笨鸟先飞＂的道理，还是有机会的。");
                }
                break;
                case 3 :
                if(str > 25)
                        say(ob->name()+"如果有意钻研，倒也不难成为一代大侠。");
                else    say(ob->name()+"应该专精几门武功，不可以贪多。");
                break;
                case 4 :
                if(per > 23 ) {
                        command("nod");
                        say(ob->name()+"天生美貌，若能继续保养，必定能长命百岁");
                }else {
                        command("shake");
                        say(ob->name()+"相貌虽然如此，若能寻些仙缘，还是可以美容的。");
                }
                break;
                case 5 :
                if(dex1 > (ob->query("age")+60) ) {
                        command("nod");
                        say(ob->name()+"身轻如燕，已达一苇渡江的能力，再下苦功，定成大器");
                }else {
                        command("shake");
                        say(ob->name()+"步伐沉重，在江湖中逃命才是重要的，练错功了啊!");
                }
                break;
        }
        me->set_temp("mimi",1);
