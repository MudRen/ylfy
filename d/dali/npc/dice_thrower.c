...
void create()
{
        set_name( "赌神", ({ "du shen","gamester" }) );
        set("long", "这是这里的职业赌手，骰子上的功夫出神入化。\n");
        set("attitude", "friendly");
        set("title", HIY "南神眼" NOR);
        set("jiali", 30);
        set("max_jing", 3000);
        set("max_qi", 3000);
        set("max_jing", 300);
        ...
        set("gender", "男性" );
		set("no_get", 1); // 防止BUG Add By Wenwu
		set("no_put", 1); // 防止BUG Add By Wenwu
        set("str", 35);
        set("con", 30);
        set("cps", 25);
        set("per",5);
...
void greeting(object ob)
{
        ...
        switch( random(10) ) {
                case 0:
                        say( "赌神笑咪咪地说道：这位" + RANK_D->query_respect(ob)
                                + "，过来赌两手，开开心吧。\n");
                        break;
                case 1:
                        say( "赌神晃了晃手中的骰子，叫道：这位" + RANK_D->query_respect(ob)
                                + "，小赌可以养家糊口，大赌可以定国安邦。来吧！\n");
                        break;
                case 2:
                        say( "赌神说道：这位" + RANK_D->query_respect(ob)
                                + "，进来！进来！ 输了算我的！\n");
                        break;
        }
        return;
...
int do_bet(string arg)
{
		...
		if( !living(this_object()) ) return notify_fail("还是等" + this_object()->name() + "醒来再说吧。\n");
...
int accept_object(object who, object ob)
{
        ...
        if(bet_type >=3)
                message = sprintf(RED"%s"NOR,chinese_number(bet_type));
        tell_object(who,"赌神悄悄告诉你：下手儿压"+message+"，一定赢！\n");
