...
#endif
#define MENGZHU NPCDATA + "meng-zhu-new"
inherit NPC;
inherit F_MASTER;
inherit F_UNIQUE;
inherit F_SAVE;
...
void create()
{
	...
	if (!restore())
	{
		set_name("魏无双", ( { "wulin mengzhu@new", "mengzhu", "zhu" }) );
		set("title", "武林盟主" );
		set("gender", "男性" );
		set("age", 40);
	...
	else
	{
		if(!query("name"))
			...
			set_name(query("name"), ( {"wulin mengzhu@new", "mengzhu", "zhu" }));
		setup();
...
int do_copy(object me, object ob)
{
	...
	me->set("short", YEL+"第" + chinese_number(me->query("generation")) + "代武林盟主" + NOR+" " + me->query("name") + "(Wulin mengzhu@new)");
	me->delete("title");
...
//            carry_object(base_name(inv[i]))->wear();
	...
	//newob = new("/d/taishan/npc/meng-zhu-new");
	//newob->move("/d/taishan/fengchan-new");
