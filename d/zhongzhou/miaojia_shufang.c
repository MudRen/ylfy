...
int do_think(string arg)
{
        ...
        if ((int)me->query_skill("unarmed", 1) < 30 ||
           (int)me->query_skill("tianchang-zhang", 1) < 30)
                return notify_fail("你发现图中所记载的掌法过于高深，自"
                                   "己无法领会到任何东西。\n");

        if ((int)me->query_skill("unarmed", 1) > 149 &&
           (int)me->query_skill("tianchang-zhang", 1) > 149)
        ...
        if ((int)me->query_skill("unarmed", 1) < 150 &&
           me->can_improve_skill("unarmed"))
                me->improve_skill("unarmed", me->query("int") * 3 / 2);

        if ((int)me->query_skill("tianchang-zhang", 1) < 150 &&
           me->can_improve_skill("tianchang-zhang"))
                me->improve_skill("tianchang-zhang", me->query("int") * 3 / 2);

        me->start_busy(random(2));
        message_vision(HIY "\n$N" HIY "聚精会神的参详图中所记载的掌法，"
                       "似有所悟。\n" NOR, me);
        write(HIC "你对「基本掌法」和「天偿掌」有了新的领悟。\n" NOR);
        return 1;
}
