...
int do_pawn(string arg)
{
	...
	if(ob->query("equipped"))
		return notify_fail("在身上穿着的装备，无法典当！！\n");
    if (ob->query("shaolin"))   
...
int do_redeem(string arg)
{
    ...
    message_vision("$N从当铺里赎回一" + ob->query("unit") + ob->query("name") + "。\n", this_player() );
    obj=new(ob);
...
string inventory_desc(object ob)
{
    ...
    else return sprintf(" (一%s)%-25s:  %s", ob->query("unit"), ob->short(), value_string(ob->query("value")));
}
int do_list()
{
            ...
            printf(NOR YEL"┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓\n"NOR);
                for (i = 0; i < sizeof(goods); i++)
                {
                                        if (replace_string(goods[i]->query("name"),NOR,"")!=goods[i]->query("name"))
                                                     ...
                                                     j=0;
                  //printf("  %-"+(30+color_len(goods[i]->short()))+"s%s每%s %s\n", goods[i]->short(),HIY+":℡ "+NOR,
				    printf("  %-30s%s每%s %s\n", goods[i]->short(),HIY+":℡ "+NOR,
                                           goods[i]->query("unit"),value_string(goods[i]->query("value")) );
    ...
    return notify_fail("目前没有可以卖的东西。\n");
        ...
        for (i=0;i<sizeof(my);i++)
        {
                     if (replace_string(inv[i]->query("name"),NOR,"")!=inv[i]->query("name"))
                        ...
                        j=0;
            printf("  %-"+(30 + color_len(inv[i]->short()))+"s：每%s%s(共有：%s%s)\n", inv[i]->short(),inv[i]->query("unit"),
                     value_string(inv[i]->query("value")),CHINESE_D->chinese_number(my[inv[i]]),inv[i]->query("unit")
            );
        }
        printf(NOR YEL"┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\n"NOR);
    return 1;
