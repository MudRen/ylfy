...
mixed suit(object ob,string arg)
{
    ...
    "armor/finger","armor/surcoat","weapon","secondary_weapon",});
...
string suit_dis(mapping suit_eff_last,mapping suit_map,mapping suit_lvl,mapping suit_count,mapping suit_skill_last)
{
    ...
    mapping suit_skill_dis=([ ]); //
          if(sizeof(suit_map))
          {
               ...
               {  
    //str+= sprintf(HIM"│"NOR"<%s>  %"+sprintf(" %d", (24+color_len(dis_count(suit_map[map[i]],suit_count[map[i]]))))+"-s【等级】:%s \n",map[i],dis_count(suit_map[map[i]],suit_count[map[i]]),dis_lvl((int)suit_lvl[map[i]]));
       str+= sprintf(HIM"│"NOR"<%s>  %-24s【等级】:%s \n",map[i],dis_count(suit_map[map[i]],suit_count[map[i]]),dis_lvl((int)suit_lvl[map[i]])); 	
               } 
...
mapping sum_eff(object ob,mapping suit_eff)
{
    ...
    {
        if ( !map_key[i] || undefinedp(eff[map_key[i]]) )
        {
            continue;
        }

        eff_str=eff[map_key[i]];
        if ( !eff_str || !mapp(eff_str) )
        {
            continue;
        }
        ...
        {
            if ( !map_str_key[k] || undefinedp(eff_str[map_str_key[k]]) )
            {
                continue;
            }
