...
void spec_recover(object me, string socket, int socket_level)
{
	...
	case "sapphire":
		if ( me->is_busy() )
		{
			if ( me->query_busy() <= socket_level*10+1 ) 
			{
			...
			{
				message_vision(HIB"$N只觉丹田一股热气慢慢升起，一处被阻塞的穴道在它的冲击下已一贯而通！\n"NOR, me);
				me->start_busy(me->query_busy()-(socket_level*10+1));
			}
		}
		break;
	case "emerald":
