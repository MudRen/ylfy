...
int do_enchase(string arg)
{
	...
	if(j==2){
	    ...
	    message_vision(CYN""+in_obj->query("name")+CYN"似乎变得更加沉稳了！\n"NOR,me);
	}
	if(j==3){
	...
	if(j==4){
	    ...
	    message_vision(CYN""+in_obj->query("name")+CYN"似乎变得更加灵活了！\n"NOR,me);
	}
	if(j==5){

	    in_obj->add("armor_prop/unarmed",k*2+random(k*5));

	    message_vision(CYN""+in_obj->query("name")+CYN"似乎变得更加充满力量了！\n"NOR,me);
	}
	if(j==6){
	...
	if(j==7){
	    ...
	    message_vision(CYN""+in_obj->query("name")+CYN"似乎变得更加充满臂力了！\n"NOR,me);
	}
