...
// 刷新时房间内物品大于下面这个数时进行清除
#define MAX_ITEM_IN_ROOM 15
// 房间内物品大于下面这个数时进行清除
#define MAX_OBJS_IN_ROOM 81
...
{ 
	return 92233720368547758; //增加room的承重
}
...
void xyzx_system_clean_up()
{
	...
	if ( sizeof(inv) < MAX_ITEM_IN_ROOM ) return;
	...
	for ( i = 0; i < sizeof(inv); i++)
	{
          if (!inv[i] || inv[i] == 0)
            continue;
		if (!destruct(inv[i]))
            log_file("destruct_2.log", sprintf("%s  %s\n", inv[i], ctime(time())));
          else
            ok++;
	}
     if (ok >=1) {
