...
LONG ); 
        ...
        set("chat_msg_combat", ({ 
                ...
                (: perform_action, "unarmed.dan" :), 
                (: perform_action, "unarmed.duo" :), 
                (: perform_action, "unarmed.huang" :), 
...
if (query("isok")==1)
{
        carry_object("/d/zhongzhou/obj/jundao"); 
...
{      
        ...
        if ((int)me->query("xyzx_sys/level") < 2000 )
        {
                command("say 你此时实力太弱，还是先打好基础再说吧。");
                return 0;
        }
        if ((int)me->query("weiwang") < 50 || (int)me->query("score") < 500000)
        {
                command("say 我不会把剑法传授给无名之辈，等你在江湖上闯出点名头来，再找我吧。");
                return 0;
        }
        /*if (skill != "miaojia-jian")
        {
                command("say 我只教你苗家剑法，别的就不要来烦我了！"); 
                return 0;
        }
