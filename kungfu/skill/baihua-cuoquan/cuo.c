...
int perform(object me, object target)
{
  string msg;
	...
	if((int)random(me->query("combat_exp")) >	(int)target->query("combat_exp")/3 || random(me->query("unarmed")) > target->query("dodge")/3)
	{
  	target->start_busy(8);
  	me->add("neili", -400);
  ...
  tell_object(me,HIG"你的防御力提高了!\n"NOR);
        me->add_temp("apply/dexerity",500);
