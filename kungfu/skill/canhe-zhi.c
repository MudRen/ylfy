...
([      "action": "$N双指并拢虚点而出，合「"NOR + MAG "天竺佛指" NOR "」及「" NOR + MAG "修罗指" NOR "」为一式，顿时一股罡风直射$n",
	"force" : 280,
	"attack": 50,
	"parry" : 50,
	"dodge" : 55,
...
([      "action": "$N双指齐点而出，合「" HIG "莲花指" NOR "」及「" HIG "多罗叶指" NOR "」为一式，顿时剑气已笼罩$n全身各处",
	"force" : 260,
	"attack": 50,
	"parry" : 50,
	"dodge" : 55,
	...
	([
    "action": "只见$N十指箕张，随手指指点点，将「" HIY "祁连五绝指" NOR "」及「" HIY "大天龙指" NOR "」的精髓招式同时使出，\n顿时剑气纵横，尘砂四起，逼向$n而去",
	    "force" : 260,
	    "attack": 50,
	    "parry" : 55,
	    "dodge" : 55,
	...
	([
    "action": "只见$N纵身跃起，长啸一声，凌空而下，「" HIM "七星剑气" NOR "」已射向$n全身各处",
	    "force" : 260,
	    "attack": 20,
	    "parry" : 25,
	    "dodge" : 25,
...
([      "action": "$N伸出两指，弹指无声，陡见两缕紫气由指尖透出，「" HIW "幽冥剑气" NOR"」射向$n周身大穴",
	"force" : 260,
	"attack": 20,
	"parry" : 30,
	"dodge" : 25,
...
([      "action": "遥见$N伸出一指轻轻拂向$n，指未到，「" HIR "参合剑气" NOR "」已将$n压得透不过气来",
	"force" : 280,
	"attack": 20,
	"parry" : 30,
	"dodge" : 35,
...
{
	return usage=="finger" || usage=="parry" || usage=="unarmed";
}
...
{
	int i, level;
	if (random(2)==0 && random(me->query_dex(1)) >= 25 && me->query_skill("canhe-zhi",1) > 200 &&
	...
	if (random(2)==0 && random(me->query_dex(1)) >= 30 && me->query_skill("canhe-zhi",1)> 200 &&
