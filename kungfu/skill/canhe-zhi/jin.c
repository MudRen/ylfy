...
int perform(object me, object target)
{
	...
	msg = HIW "\n只见$N" HIW "十指分摊，霎时破空声骤响，数股剑气至指尖激射而出，朝$n" HIW "径直奔去！\n" NOR;
	...
	{
		msg += CYN "\n$n" CYN "见$N" CYN "来势汹涌，急忙飞身一跃而起，避开了这一击。\n" NOR;
	}
	...
	else if (ap * 2 / 3 + random(ap) > dp)
	{
		...
		msg +=HIR "\n只听$n" HIR "一声惨嚎，被$N的剑气刺中了要害，血肉模糊，鲜血迸流不止。\n" NOR;
	...
	else if (ap * 2 / 3 + random(ap) > dp)
	{
		...
		msg +=HIR "\n$n" HIR "奋力招架，仍是不敌，$N的" HIR "无形剑气已透体而入，鲜血飞射，无力再战。\n" NOR;
	}
	else
	{
		msg += CYN "\n$n" CYN "见$N" CYN "来势汹涌，急忙飞身一跃而起，避开了这一击。\n" NOR;
	...
	msg = HIW "\n最后 $N" HIW "食指伸出，施展出参合指的精妙招式，一道气流袭向$n的胸前穴位。\n" NOR;
	...
	if (ap / 2 + random(ap) > dp)
	{
		...
		msg += HIR "$n" HIR "躲避不及，顿时觉得顿觉浑身经脉都是一痛，手里不禁一软。\n" NOR;
	}
	else
	{
		me->add("neili",-40);
		msg += CYN "可是$n" CYN "看破了$N" CYN "的来路，立刻腾挪躲闪，使$N的攻击没有起到作用。\n" NOR;
		me->start_busy(3);
	}
	message_vision(msg, me, target);
