...
int perform(object me, object target)
{
	...
	me->add("neili", -250);
	if(random(me->query("xyzx_sys/level")) > (int)target->query("xyzx_sys/level")/5*3 && me->query_str(1) > (int)target->query_str(1)/3*2)
	{
	...
	else if(random(me->query("xyzx_sys/level")) > (int)target->query("xyzx_sys/level")/3 && me->query_str(1) > (int)target->query_str(1)/2)
	{
...
// no condition fs_poison, change to xx_poison (by xbd)
		...
		target->apply_condition("xx_poison", random(100) + (me->query_skill("poison", 1)/10) + target->query_condition("xx_poison"));
		me->start_busy(5);
		target->start_busy(1);
		return 1;
	}
...
// no condition fs_poison, change to xx_poison (by xbd)
	me->apply_condition("xx_poison", random(3) + (me->query_skill("poison", 1)/10) + me->query_condition("xx_poison"));
	return 1;
}
