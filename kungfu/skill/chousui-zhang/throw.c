...
int perform(object me, object target)
{
	...
	if( !living(target) && userp(target) && target->query("xyzx_sys/level")/2 > me->query("xyzx_sys/level") )
		return notify_fail("你想要杀此人，但他比你强太多，你不敢动手。\n");

//pk
	if( userp(target) && me->query_condition("killer"))
		return notify_fail("你已经被通辑了，不要再乱杀玩家了！\n");
	if( userp(target) && target->query("xyzx_sys/level") < me->query("xyzx_sys/level") && me->query_condition("killer"))
		return notify_fail("你已经被通辑了，不要再乱杀玩家了！\n");
...
//免战系统
	        ...
	        > random(target->query_skill("dodge") ) )
	{
		...
		target->apply_condition("xx_poison", random(me->query_skill("chousui-zhang",1)/10) + 10 + target->query_condition("xx_poison"));
