...
int get_finish(object me)
{
        ...
        if (me->query("kar") < 31)
        {
                tell_object(me, "你演练完毕，只觉依自己的福源运气性，根"
                                "本无法将二者合一。\n");
                return 0;
        }

        if (me->query_skill("literate", 1) < 7000)
        {
      ...
      /*  if (me->query_skill("martial-cognize", 1) < 250)
        ...
        {
                tell_object(me, "你觉得自己内力不继，无法将苗家剑法和胡"
                                "家刀法二者合一。\n");
                return 0;
        }
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        if (damage_bonus < victim->query_int(1))
                return 0;

        // if (me->query_temp("daojian-guizhen/max_pfm"))
        {
                victim->receive_wound("qi", (damage_bonus - 80) / 2+10, me);
                return random(2) ? HIR "只听「噗嗤」一声，一股鲜血至$n胸前射出。\n" NOR:

                                   HIR "霎时只见寒光一闪，一柱鲜血至$n胸前射出。\n" NOR;
        }
}
...
mixed ob_hit(object ob, object me, int damage, object weapon)
{
        ...
        if (ap / 2 + random(ap) < dp || random(8)==0)
        {
                ...
                result += (["msg" : HIC "$n" HIC "突然使出一招「大雪纷纷」，发出万长白芒，寒光四射，直劈向$N化守势为攻势，对$N发动进攻！\n" NOR]);
                        msg=HIC "$N" HIC "突然使出一招「大雪纷纷」，发出万长白芒，寒光四射，直劈向$n化守势为攻势，对$n发动进攻！\n" NOR;
