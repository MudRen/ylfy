...
int perform(object me, object target)
{
	...
	attack = me->query("xyzx_sys/level");
	attack += me->query_skill("unarmed");
	attack += me->query("neili")/5;
	defense = target->query("xyzx_sys/level");
	defense += target->query_skill("dodge");
	defense += target->query("neili")/7;
	attack = (attack+random(attack+1))/2;

	damage = me->query_skill("dashou-yin") + me->query_skill("force") + jiali;

	message_vision(BLU"\n$N突然面色怪异，低声默念密宗真言，双臂骨节一阵爆响，猛然腾空而起，伸手向$n胸前按去，好一式密宗「大手印」！\n"NOR,me,target);

	if( attack > defense )
	{
		if( objectp(armor = target->query_temp("armor/cloth")) && armor->query("armor_prop/armor") < 1500 && damage > 500)
		{
		...
		target->apply_condition("dashouyin", 1+(me->query_str(1)/8));
