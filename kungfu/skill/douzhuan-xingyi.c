...
if (me->query_skill("shenyuan-gong",1) < 100) return damage;
...
if (me->query_skill("shenyuan-gong",1) < 100) return;
	...
	case 5 :
		if ( objectp( me->query_temp("weapon")) ) {
			if ( me->query("gender") == "男性")
				...
				msg += HIC"$N突然使出华山绝技，一式“无双无对，宁式一剑”，只见$N挺剑直出，剑式如虹，嗤嗤之声大作，凌厉绝伦，剑力已将$n全身笼罩。\n"NOR;
...
if (ob->query("qi")/2 > damage/2)
	...
	case 16 :
		if ( objectp( me->query_temp("weapon")) ) {
			msg += HIR"\n$N轻描淡写，挥尘如意，一瞬间就已使出夺命十三式，手中的武器轻灵流动 ，就像是河水般的向$n刺出,天地间彷佛充满了杀气！\n"NOR;
if (ob->query("qi")/2 > damage)
			ob->receive_wound("qi", damage);
		}
		else {
