...
int perform(object me, object target)
{
	...
	if(!k||k==0)	k=2;
	if(random(me->query("xyzx_sys/level"))>target->query("xyzx_sys/level")/k)	{
	...
	inv=all_inventory(target);
		...
		if( (thing->query("equipped")&&random(me->query("str"))<20) || (int)thing->query("no_steal"))
