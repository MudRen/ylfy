...
int perform(object me, object target)
{
	...
	if(extra<100) 
		return notify_fail("你陆奥圆明流拳术的修为不够，无法使出紫电四连脚的绝招。\n");
	...
	message_vision(HIY"$N突然一笑，右手在$n的眼前一晃，跃向另一侧。$n一时无法看清$N的身影。 「紫电四连脚！」 \n",me,target);
...
{
	string msg;
	if( !target || !target->is_character() || !me->is_fighting(target) || environment(target)!=environment(me) )
		return 0;
