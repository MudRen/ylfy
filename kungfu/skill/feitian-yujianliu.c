...
mapping query_action(object me, object weapon)
{
	...
	{
		return ([
            "action" : HIW"「 飞翔龙闪! 」"+HIY"$N突然消失了！随后$w的反光出现在了$n的头上。"+HIY"$N尽然在半空使出了拔刀术。 "NOR,
		            "force" : 650,
		            "dodge" : 50,
		            "damage": 650,
            "skill_name" : "飞翔龙闪",
...
int practice_skill(object me)
{
	...
	if (me->query_skill("blade", 1) <=me->query_skill("feitian-yujianliu", 1))
...
//if (!level) return;
	if (!level2) return;
	if (!jiali) jiali=10;
	if( damage_bonus > 60 && random(level2)>150 && random(2)==0 )
	{
