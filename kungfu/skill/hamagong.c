...
int valid_enable(string usage)
{
	...
	if (lvl >= 300)
		return usage == "unarmed" || usage == "force" || usage =="parry" || usage == "strike";
	else
		return usage == "force";
}
int can_not_hubo()
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	if( random( me->query_skill("force") ) > random( victim->query_skill("force")) && random(3)==0 )
	{
		if(me->query_skill("hamagong", 1)>150)
		{
			if (me->query_skill_mapped("force") == "hamagong" && !me->query_temp("weapon"))
			{
				victim->receive_wound("qi", (damage_bonus ) / 2 );
	...
	if (random( me->query_skill("force") ) > random( victim->query_skill("force")) && random(2)==0)
	{
		me->add("neili", -30);
		victim->receive_wound("qi", (damage_bonus - 90) / 3+20);
		return WHT "$n" WHT "硬承下$N" WHT "一掌，霎时全身一颤，经脉受震，喷出一口鲜血！\n" NOR;
	}
}
