...
int perform(object me, object target, int improve)
{
	...
	improve =(int)me->query_skill("huashan-jianfa", 1) / 5;
	if( !target ) target = offensive_target(me);
	if( !target || !target->is_character() ||!me->is_fighting(target) )
		return notify_fail("无双无对只能在战斗中对对手使用。\n");

	if (!weapon || weapon->query("skill_type") != "sword" || me->query_skill_mapped("sword") != "huashan-jianfa")
	...
	message_vision(HIC"\n$N突然使出华山绝技，一式“无双无对，宁式一剑”，只见$N挺剑直出，剑式如虹，嗤嗤之声大作，凌厉绝伦，剑力已将$n全身笼罩。\n"NOR, me,target);
	me->start_busy(2);
