...
|| me->query_skill_mapped("parry") != "hujia-dao")	
		return notify_fail("你现在无法使用「双刀合璧」。\n");

	if(!weapon1 || weapon1->query("skill_type") != "blade"
|| target->query_skill_mapped("blade") != "hujia-dao"	
|| target->query_skill_mapped("parry") != "hujia-dao")	
		return notify_fail(target->name()+"现在无法使用「双刀合璧」。\n");

skill = me->query_skill("hujia-dao", 1);	
skill1 = target->query_skill("hujia-dao", 1);	
