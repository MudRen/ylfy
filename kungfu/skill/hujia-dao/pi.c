...
int perform(object me, object target)
{
	...
	if ((int)me->query_skill("hujia-dao", 1) < 2090)
	...
	if (me->query_temp("hu/freeze") || random(1)==0)
	{
		msg += HIW + weapon->name() + "寒气逼人，刀身撕裂空气，片片雪花飘落，刀未至气先及，$n" HIW "眉毛都蒙上了薄薄白霜！\n\n"NOR;
	}
	...
	if (ap / 2 + random(ap) > dp)
	{
		...
		{
			msg += HIR "$n" HIR "见状不妙，连忙举起" + weapon2->name() + "抵挡，在$N这霸气无边的一招下，" + weapon2->name() +"给震成碎片！\n" NOR;
			...
			msg += HIR + weapon->name() + HIR "余势未尽，又劈入了$n的胸口，$n遭此重创，鲜血狂喷而出！\n" NOR;
			...
			if (me->query_temp("hu/freeze")  || random(3)==0)
			{
				...
				msg += HIR "\n$n" HIR "身形摇晃间，感觉到" + weapon->name() +"上一阵刺骨的寒气侵入体内，复又喷出一口鲜血！\n" NOR;
			}
		}
