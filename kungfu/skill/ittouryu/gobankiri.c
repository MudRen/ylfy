...
int perform(object me, object target)
{
        ...
        myexp = (int)me->query("xyzx_sys/level");
        hisexp= (int)target->query("xyzx_sys/level");
        myweapon = me->query_temp("weapon");
        weapon_name= myweapon->query("name");
        
        if(random(hisexp*5) < (extra*extra*extra/4+myexp+hisexp)/3  && weapon)  
        {
        message_vision( HIG "$N根本无视$n的攻击，大吼一声「棋盘切」！将"+weapon_name+"高举过肩，自右上方直向$n劈来。面对$N的雷霆一般的一击，$n用手中的兵器下意识地一挡。
                      "+ HIW"$n只觉得虎口一热，手中的"+weapon->name()+"再也拿不住了，“当”地一声掉在地上。\n\n" NOR, me,target);
        if(weapon->unequip()) weapon->move(environment(target));
        me->start_busy(1);
        }
        else if(random(hisexp*3) < (extra*extra*extra/4+myexp+hisexp)/3  && weapon)
        { 
        msg = HIG "$N根本无视$n的攻击，大吼一声「棋盘切」！将"+weapon_name+"高举过肩，自右上方直向$n劈来。面对$N的雷霆一般的一击，$n用手中的兵器下意识地一挡。"+
        ...
        {
        msg = HIG "$N根本无视$n的攻击，大吼一声「棋盘切」！将$w高举过肩，自右上方直向$n劈来。"NOR;
