...
int perform(object me, string arg)
{
	...
	if(!arg)
		return notify_fail("你想和谁一起双剑合璧？\n");
	if(!objectp(target = present(arg, environment(me))))
		return notify_fail("这里没有这个人。\n");

	if( !victim ) victim = offensive_target(me);

	if( !victim || !victim->is_character() || !me->is_fighting(victim) || !living(target) )
		return notify_fail("双剑和璧左右互搏只能对战斗中的对手使用。\n");

	if(target==me)
		return notify_fail("想和自己合璧？\n");
	...
	if(!weapon || weapon->query("skill_type") != "sword" || me->query_skill_mapped("sword") != "liangyi-jian" )
	...
	if(!weapon1 || weapon1->query("skill_type") != "sword" || target->query_skill_mapped("sword") != "liangyi-jian")
	...
	if(target->query_temp("prepare_gmhebi"))
	{
		...
		message_vision(HIC "$N"HIC"和$n" HIC "两人眼神一交，心意已然相通，不由的会心一笑，只见二人身形一错，霎时\n"
