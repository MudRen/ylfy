...
([  "name":   "少商剑",
    "action": "$N大拇指一按，嗤嗤两指，劲道使得甚巧，「" HIW "少商剑" NOR "」剑气如怒潮般涌至",
	"force" : 360,
	"attack": 30,
	"dodge" : 30,
	"parry" : 30,
...
([  "name":   "少商剑",
    "action": "$N大拇指连挥，「" HIW "少商剑" NOR "」便如是一幅泼墨山水，纵横倚斜，剑路雄劲",
	"force" : 360,
	"attack": 30,
	"dodge" : 30,
	"parry" : 30,
...
([   "name":   "少商剑",
    "action": "$N双手拇指同时捺出，「" HIW "少商剑" NOR "」大开大阖，气派宏伟，每一剑都有风雨大至之势",
	"force" : 360,
	"attack": 30,
	"dodge" : 30,
	"parry" : 30,
...
([  "name":   "商阳剑",
    "action": "$N变招奇速，右手食指疾从袖底穿出，「" HIW "商阳剑" NOR "」登时幻出无数指影",
	"force" : 340,
	"attack": 35,
	"dodge" : 30,
	"parry" : 35,
...
([   "name":   "商阳剑",
    "action": "$N拇指一屈，食指随即点出，嗤嗤两声急响，变成商阳剑法，「" HIW "商阳剑" NOR "」激射刺出",
	"force" : 440,
	"attack": 35,
	"dodge" : 30,
	"parry" : 35,
...
([      "name":   "中冲剑",
    "action": "$N将中指向上一刺，「" HIW "中冲剑" NOR "」拔地而起，接着手指向下一划，剑气如利刀般砍出",
	"force" : 560,
	"attack": 35,
	"dodge" : 10,
	"parry" : 30,
...
([      "name":   "中冲剑",
   "action": "电光火石之间，$N猛然翻掌，右手陡然探出，中指「" HIW "中冲剑" NOR"」向$n一竖",
	"force" : 360,
	"attack": 35,
	"dodge" : 10,
	"parry" : 30,
...
([  "name":   "关冲剑",
    "action": "$N右手无名指伸出，「" HIW "关冲剑" NOR "」剑路拙滞古朴，一股雄浑无比的内力鼓荡而出",
	"force" : 330,
	"attack": 30,
	"dodge" : 30,
	"parry" : 35,
...
([  "name":   "关冲剑",
    "action": "$N俯身斜倚，无名指「" HIW "关冲剑" NOR "」弹射而出，指尖已对准$n发出了一缕强烈的劲风",
	"force" : 330,
	"attack": 30,
	"dodge" : 30,
	"parry" : 95,
...
([  "name":   "关冲剑",
    "action": "$N无名指轻轻一挥，「嗤啦」一声，拙滞古朴的「" HIW "关冲剑" NOR "」剑气向$n直射而出",
	"force" : 330,
	"attack": 30,
	"dodge" : 30,
	"parry" : 95,
...
([  "name":   "少泽剑",
    "action": "忽见$N左手小指一伸，一条气流从$P少冲穴中激射而出，一股「" HIW "少泽剑" NOR "」登时射向$n",
	"force" : 500,
	"attack": 30,
	"dodge" : 35,
	"parry" : 32,
...
([      "name":   "少冲剑",
    "action": "$N掌托于胸前，伸出右小指，一招「" HIW "少冲剑" NOR "」缓缓地点向$n的周身大穴",
	"force" : 530,
	"attack": 30,
	"dodge" : 30,
	"parry" : 35,
...
([  "name":   "少冲剑",
    "action": "$N小指一弹，「" HIW "少冲剑" NOR "」化式「分花拂柳」，剑势如同柳絮一般，飘而不乱",
	"force" : 430,
	"attack": 30,
	"dodge" : 30,
	"parry" : 35,
...
([  "name":   "少冲剑",
    "action": "$N一招「" HIW "少冲剑" NOR "」，剑气回转无定形，竟从左侧绕了过来，点向$n",
	"force" : 530,
	"attack": 50,
	"dodge" : 50,
	"parry" : 55,
...
([  "name":   "少冲剑",
    "action": "$N右手小指一挥，一招「" HIW "少冲剑" NOR "」点点刺刺破空刺出，宛如雕花刺画一般",
	"force" : 530,
	"attack": 80,
	"dodge" : 50,
	"parry" : 55,
...
{
	return usage=="finger" ||  usage=="unarmed"||  usage=="parry";
}
...
{
	if (me->query_skill("kurong-changong",1) > 10 && me->query("neili") > 120)
	{
...
mapping query_action(object me, object weapon)
{
	...
	if (random(me->query_skill("liumai-shenjian",1)) > 180 &&
	        me->query_skill("kumu-shengong",1) > 180 &&
	        me->query("neili") > 120
	...
	if ( lvl_temp > 100 && me->query_skill("kumu-shengong",1) > 100  && me->query("neili") > 100
	        && me->query_skill_mapped("force") == "kumu-shengong")
	{
		if ( lvl_temp <= 140)
			return ([
		...
		if ( lvl_temp > 150 && lvl_temp <180  && me->query_skill("kumu-shengong",1) > 100  && me->query("neili") > 100
		        && me->query_skill_mapped("force") == "kumu-shengong")
			return ([
		...
		if ( lvl_temp >= 180  && me->query_skill("kumu-shengong",1) > 100  && me->query("neili") > 100
		        && me->query_skill_mapped("force") == "kumu-shengong")
			return ([
