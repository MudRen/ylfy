...
{
    int skill;
    if( target != me ) return notify_fail("你只能用龙门神功提升自己的战斗力。\n");
	...
	if (skill<60) return notify_fail("你的龙门神功还不够娴熟。\n");
    me->add("neili", -100);
    me->receive_damage("qi", 0);
    message_vision( HIC "$N大吼一声，运起龙门神功已将全身潜力尽数提起！\n" NOR, me);
...
void remove_effect(object me, int amount)
{
   ...
   if ( (int)me->query_temp("powerup") ) {
    ...
    tell_object(me, "你的龙门神功运行完毕，将内力收回丹田。\n");}
}
