...
int valid_learn(object me)
{
	...
	if( (int)me->query("max_neili") < 50 )
		return notify_fail("你的内力不够，没有办法练回风落雁剑。\n");
	if( (string)me->query_skill_mapped("force")!= "huiyan-xinfa")
		return notify_fail("回风落雁剑必须配合回雁心法才能练。\n");
	...
	if( !(ob = me->query_temp("weapon")) || (string)ob->query("skill_type") != "sword" )
		return notify_fail("你必须先找一把剑才能练回风落雁剑。\n");
...
int yunwu(object me, object victim, object weapon, int damage)
{
	...
	else if ( !living(victim) && victim->query_skill("force",1) > 100 )
	{
