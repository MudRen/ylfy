...
//对黑暗属性目标强大
    ...
    {
        damagic=168000+damage*6;
            target->receive_damage("qi", damagic);
        damagic=168000+damage*3;
            target->receive_wound("qi", damagic);
    }
    else 
     {
	    damagic=96000+damage*2;
	    target->receive_damage("qi", 90600+damagic*2);
	    target->receive_wound("qi", 96000+damagic);
	    }
	if(target->is_ghost()==1 && !userp(target)) {
