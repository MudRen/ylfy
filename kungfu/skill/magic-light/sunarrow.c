...
//对光属性目标无作用
    if ((int)target->query("magicgift")==6)
        {
            damagic=30000+damage;
                target->receive_damage("qi", damagic);
    if (wizardp(me))
        tell_object(me, "\n基本杀伤为"+damagic+"点。\n" NOR);

        damagic=20000+damage;
                target->receive_wound("qi", damagic);
        }
//对暗属性目标强大
    else  if ((int)target->query("magicgift")==5)
        {
        damagic=69000+damage*6;
            target->receive_damage("qi", damagic);
    if (wizardp(me))
        tell_object(me, "\n基本杀伤为"+damagic+"点。\n" NOR);
         damagic=69000+damage*3;
                target->receive_wound("qi", damagic);
    }
else 
{
damagic=20000+damage*3;
target->receive_damage("qi", 20000+damage*3);
target->receive_wound("qi", 20000+damage*2);
if (wizardp(me))
tell_object(me, "\n基本杀伤为"+damagic+"点。\n" NOR);
}
    target->start_busy(1);
        ...
        {
       damagic=15000+damage*2;
                target->receive_damage("qi", damagic);
        damagic=15000+damage;
                target->receive_wound("qi", damagic);
if (wizardp(me))
tell_object(me, "\n基本杀伤为"+damagic+"点。\n" NOR);
	    msg += HIY"                                               \n"NOR;
