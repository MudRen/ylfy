...
//对光属性目标无作用
    if ((int)target->query("magicgift")==6)
        {
        damagic=400+damage;
                target->receive_damage("qi", target->query("eff_qi")/1000);
        damagic=350+damage;
                target->receive_wound("qi", target->query("eff_qi")/1000);
        }
...
//对黑暗属性目标强大
        ...
        damagic=1800+damage*3;
                target->receive_damage("qi", target->query("eff_qi")/300);
        damagic=1800+damage;
                target->receive_wound("qi", target->query("eff_qi")/300);
           }
    else 
   {
        damagic=500+damage;
        target->receive_damage("qi", target->query("eff_qi")/500);
        target->receive_wound("qi", target->query("eff_qi")/500);
    }
