...
//对光属性目标无作用
    ...
    {
        damagic=6000+damage;
                target->receive_damage("qi", target->query("eff_qi")/10);
        damagic=3500+damage;
                target->receive_wound("qi", target->query("eff_qi")/10);
    if (wizardp(me))
        tell_object(me, "\n基本杀伤为"+damagic+"点。\n" NOR);
    }
//对黑暗属性目标强大
    ...
    if (wizardp(me))
     ...
     {
    damagic=9000+damage;
    target->receive_damage("qi", target->query("eff_qi")/5);
    target->receive_wound("qi", target->query("eff_qi")/5);
