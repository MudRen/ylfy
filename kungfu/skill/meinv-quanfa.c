...
([      "action" : "$N双掌连拍数下，接著连绵不断拍出，原来是"+(order[random(13)])+"「文姬归汉」"NOR"，共胡笳十八拍",
	"force" : 270,
	"dodge" : 115,
	"damage": 160,
	"lvl" : 34,
...
([      "action" : "$N忽然昂首如吟明月，双掌从不可思议的角度攻了过来，原来是一招"+(order[random(13)])+"「班姬赋诗」"NOR"",
	"force" : 450,
	"dodge" : 115,
	"damage": 194,
	"lvl" : 71,
...
([      "action" : "$N五指在自己头发上一梳，跟著软软的挥了出去，脸上微微一笑，却是一招"+(order[random(13)])+"「丽华梳装」"NOR"。",
	"force" : 530,
	"dodge" : 220,
	"damage": 210,
	"lvl" : 95,
...
([      "action" : "$N脚下翩若惊鸦、矫若游龙，犹如在水上漂行一般，却是一招"+(order[random(13)])+"「洛神微步」"NOR"",
	"force" : 530,
	"dodge" : 120,
	"damage": 250,
	"lvl" : 149,
...
([      "action" : "$N使招"+(order[random(13)])+"「曹令割鼻」"NOR"，挥手在自己脸上斜削一掌，左掌削过，右掌又削，连绵不断",
	"force" : 530,
	"dodge" : 120,
	"damage": 300,
	"lvl" : 179,
...
([      "action" : "$N左手斜举，右手五指弹起，仿似弹习琵琶一般，"+(order[random(13)])+"「昭君出塞」"NOR"五指轮番向$n弹去",
	"force" : 300,
	"dodge" : 20,
	"damage": 211,
	"lvl" : 25,
...
([      "action" : "$N施出"+(order[random(13)])+"「天女织梭」"NOR"，右手挥左，左手送右，做投梭织布之状，一挥一送，击向$l",
	"force" : 370,
	"dodge" : 10,
	"damage": 211,
	"lvl" : 45,
...
([      "action" : "$N脸露微笑，伸手往头上一梳，手指如"+(order[random(13)])+"「丽华梳妆」"NOR"，软软挥将出去，拍向$n的胸口",
	"force" : 250,
	"dodge" : 25,
    "skill_name" : "丽华梳妆",
	"lvl" : 65,
...
([      "action" : "$N一招"+(order[random(13)])+"「弄玉吹箫」"NOR"，一指擎天，一指对地，闭目垂首，忽然手腿齐出，攻向$n全身要害",
	"force" : 500,
	"dodge" : 5,
	"damage": 311,
	"lvl" : 85,
...
int valid_learn(object me)
{
	...
	if ((int)me->query_skill("yunv-xinjing", 1) < 10)
...
{
	int i, level;
	level = (int) me->query_skill("meinv-quanfa",1);
...
{
	int i,l;
	l = (int)me->query_skill("meinv-quanfa", 1) / 4;
	i=me->query("per");
	i=i-20;
	if (i=0) i=2;
	if (i<0) return 0;
	if (me->query("gender") == "无性") return 0;
	if(!random(5) && me->query_skill_mapped("parry") == "meinv-quanfa" && (me->query("neili")>200) && me->query_skill("meinv-quanfa", 1) > 60)
	{
		if (me->query("gender") == "女性")
		{
		...
		if((me->query("neili") > 60) && (!victim->query_temp("yunv/beauty")) )
		{
...
{
	return __DIR__"meinv-quanfa/" + action;
}
