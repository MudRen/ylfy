...
int perform(object me, object target)
{
	...
	if( random(me->query_skill("unarmed")) > random((int)target->query_skill("dodge")* 2 / 3 ))
	{
		...
		if(target->query_skill("hunyuan-yiqi",1)>me->query_skill("qianzhu-wandushou",1)*12/10&&random(2)==0)
		{
			...
			damage=(int)me->query_skill("unarmed")+(int)me->query_skill("force");
		...
		else
		{
			...
			damage=(int)me->query_skill("duji",1)
			       +(int)me->query_skill("qianzhu-wandushou")
			       +(int)me->query_skill("wudu-shengong");
