...
int perform(object me,object target)
{
    ...
    if( my_skill < 50 ) return notify_fail("你的千蛛万毒手火候太浅，不能用来吸取毒素！\n");
    if( objectp(me->query_temp("weapon")) ) return notify_fail("你必须空手才能修炼千蛛万毒手！\n");
    if( me->is_fighting() ) return notify_fail("战斗中无法修炼千蛛万毒手！\n");
    if( me->is_busy() ) return notify_fail("你正忙着呢！\n");
    if( target->is_fighting() || target->is_busy() ) return notify_fail("毒虫正忙着呢，不能和你配合！\n");
    if( me->query_temp("wudu_suck") ) return notify_fail("你正在修炼中！\n");
    if( my_skill > 100 && my_skill < 160 && tg_age < 999 ) return notify_fail(target->query("name")+"的毒力对你来说已经太轻微了！\n");
	if( my_skill > 159 && tg_age < 9999 ) return notify_fail(target->query("name")+"的毒力对你来说已经太轻微了！\n");
    if( (int)me->query("neili") < 200 ) return notify_fail("你的内力不够，不足以对抗毒气，别把小命送掉。\n");
    if( (int)target->query("qi") < 50 ) return notify_fail( target->query("name") + "已经这样了，你无法从他体内吸取任何毒素！\n");
    if( (int)me->query("qi") < 50 ) return notify_fail( "你快不行了，再练会送命的！\n");
...
void del_wudusuck(object me,object target)
{
	...
	if( me->query_temp("wudu_suck") )
	{
		...
		message_vision(HIG"只见$N的肚子越涨越大，“吧嗒”一声，松开口掉在地上。\n" NOR, target );
