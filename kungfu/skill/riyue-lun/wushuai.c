		...
		switch(times) {
			case 0:
				...
				if (random(me->query_skill("hammer")) > target->query_skill("parry") /2 ) {
					...
					damage = damage + random(damage)*2*(200+lx);
			...
			case 2:
				msg=msga[random(sizeof(msga))];
				if (random(me->query("xyzx_sys/level")) > target->query("xyzx_sys/level",1) /2 ) {
					damage = (int)me->query_skill("riyue-lun",1) + (int)me->query_skill("longxiang",1);
					damage = damage + random(damage)*2*(200+lx);
					target->receive_damage("qi", damage);
					target->receive_wound("qi", damage/2);
			...
			case 3:
 			...
 			case 4:
				msg=msga[random(sizeof(msga))];
				if ( random(me->query_con(1)+me->query_str(1))*3 > target->query_dex(1) ) {
					damage = (int)me->query_skill("riyue-lun",1) + (int)me->query_skill("longxiang",1);
					damage = damage/2 + random(damage)*(300+lx);
					target->receive_damage("qi", damage);
					target->receive_wound("qi", damage/2);
