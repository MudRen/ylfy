...
//                return notify_fail("你必须使用圣火令或铁焰令来传递阴风刀内劲！\n");     
      ...
      if(random(me->query_skill("sword")) > random(target->query_skill("parry"))){                
           damage = me->query_skill("sowrd");
           damage = damage + random(damage);
           if(target->query_skill("sowrd")-50 > me->query_skill("parry"))
             damage = damage/2;                  
           if(target->query_skill("sword") > me->query_skill("parry")*2)
             damage = random(50);
           if(me->query_skill("sword") > target->query_skill("parry")*2)
             damage = damage+random(damage);
