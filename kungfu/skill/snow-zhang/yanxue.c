...
int perform(object me, object target)
{
   ...
   return notify_fail("你的内力修为还不够高。\n");
    ...
    msg = HIW "$N冰雪心法运转一周天，全身只觉一股寒气蒸腾欲出,清啸一声,\n"
    ...
    if( random(me->query("xyzx_sys/level")) > target->query("xyzx_sys/level")/3) { 
     ...
     damage = (int)me->query_skill("snow-zhang")+(int)me->query_skill("bingxue-xinfa");
   ...
   me->add("neili", -300);
   msg += HIC"$n只见漫天青气席卷而来,躲无可躲,卒一接触,便被震的七窍流血,如断线风筝向后飘去。\n"NOR;
    } else
    {
