...
int perform(object me, object target)
{
	...
	if( (int)me->query_skill("hanbing-zhenqi", 1) < 100 )
		return notify_fail("你的寒冰真气火候不够，不会使用「峻极神掌」！\n");

	if( me->query_skill_mapped("force") != "hanbing-zhenqi" )
		return notify_fail("你没有激发寒冰真气，无法使用「峻极神掌」！\n");

	if ( me->query_skill_mapped("unarmed") != "songyang-zhang")
	...
	if( random(me->query_skill("unarmed")) < target->query_skill("force")/2 || neili < neili2/2)
	{
	...
	else if( me->query_skill("unarmed") < target->query_skill("force")/3*2 || neili < neili2/3*2)
