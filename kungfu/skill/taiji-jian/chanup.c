...
if (!me->query("breakup"))		
return notify_fail("你任督二脉未通,无法使用["+HIR+"缠"+NOR+"]字诀\n"NOR,); 		
            }
        me->add("neili", -cost);
        me->add("jing", -10);
...
if(target->is_busy()) target->start_busy(20);	
           me->start_busy(1);
           me->delete_temp("tjj/chan");
...
if(target->is_busy()) target->start_busy(30);	
           me->start_busy(1);
           me->delete_temp("tjj/chan");
           return 1;
           }   
...
int checking(object me, object target, object weapon)
{
        ...
        ap = me->query("combat_exp")/1000 * me->query_skill("taiji-jian",1) * me->query_int(1);
        dp = target->query("combat_exp")/1000 * target->query_skill("dodge",1) * target->query_int(1);
        if (ap < 1) ap = 1;
        if (dp < 1) dp = 1;
