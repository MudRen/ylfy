...
int perform(object me, object target)
{
	...
	if ((int)me->query_skill("taiji-quan", 1) < 850)
	...
	if (!(int)me->query("breakup"))
        return notify_fail("你任督二脉未通,难以施展" TU "。\n");
	...
	if (me->query_skill_mapped("parry") != "taiji-quan")
		return notify_fail("你现在没有激发太极拳在招架上，难以施展" TU "。\n");

	if (me->query_skill_mapped("force") != "taiji-shengong")
		return notify_fail("你现在没有激发太极神功，难以施展" TU "。\n");

	if ((int)me->query("jing") < 3000)
	...
	for (flag = 0, i = 0; i < sizeof(obs); i++)
	{
		...
		if (ap + random(ap) > dp)
		{
			...
			obs[i]->receive_wound("jing", damage / 3);
	...
	if (! flag)
		message_vision(HIM "然而没有任何人受了$N的影响。\n\n" NOR, me, 0, obs);
