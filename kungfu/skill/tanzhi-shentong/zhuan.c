...
int perform(object me, object target)
{
	...
	msg = HIC "$N" HIC "将全身功力聚于一指，指劲按照二十八宿方位云贯而出，正是桃花岛「" HIR "转乾坤" HIC "」绝技。\n" NOR;
	...
	else
	{
		me->start_busy(3);
		me->add("neili", -300);
		msg += CYN "$p" CYN "见$P" CYN "招式奇特，不感大意，顿时向后跃数丈，躲闪开来。\n" NOR;
	}
	...
	if (ap * 3 / 2 + random(ap) > dp)
	{
		...
		msg += HIR "$n" HIR "只觉$N" HIR "指风袭体，随即上体一阵冰凉，顿感真气涣散几欲晕厥。\n" NOR;
		me->start_busy(3);
	}
	else
	{
		msg += CYN "可是$p" CYN "防守严密，紧守门户，顿时令$P"CYN "的攻势化为乌有。\n" NOR;
		me->start_busy(4);
	}
	message_vision(msg, me, target);
	msg = HIC "\n最后$N" HIC "合指轻弹，只听破空声大作，一枚石子疾速飞出" + "，打向$n" HIC "。\n" NOR;
	...
	{
		msg += HIR "石子来得好快，$n"HIR + "一个闪避不及，被打了个正中，浑身一阵剧痛，整个人几乎都要散架了。\n" NOR;
