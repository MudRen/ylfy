...
int perform(object me, object target)
{
	...
	if ((int)me->query_skill("tianmo-jian", 1) < 180)
   ...
   if (! living(target))
	...
	{
		msg += HIR "$n" HIR "顿时只觉得头晕目眩，天旋地转，一时难以招架。\n" NOR;
                count = ap / 16;
                attack_time += random(ap / 45);
        } else
        {
                msg += HIC "$n" HIC "见$N" HIC "剑势汹涌，急忙凝神抵挡，不为所困。\n" NOR;
                count = 0;
        }                
