...
int exert(object me, object target)
{
	...
	if( target != me )
		return notify_fail("你只能用天魔神功来提升自己的战斗力。\n");
	...
	if (me->query_skill("tianmo-shengong",1) < 100)
		return notify_fail("你的天魔神功还不够娴熟。\n");

	skill = me->query_skill("force");
	me->add("neili", -100);
	me->receive_damage("qi", 0);

	message_vision(HIY "$N运起天魔神功，全身骨骼叻嘞作响，肌肉贲张，体形暴涨近半，全身发出金色豪光。\n" NOR, me);
...
void remove_effect(object me, int amount)
{
	...
	if ( (int)me->query_temp("powerup") )
	{
		...
		tell_object(me, "你的天魔神功运行完毕，全身筋肉缓缓恢复原样。\n");
