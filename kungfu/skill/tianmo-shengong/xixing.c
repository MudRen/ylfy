...
int exert(object me, object target)
{
        ...
        int sp, dp;
        int my_max, tg_max;
        sp = me->query_skill("force") + me->query_skill("dodge") + me->query_skill("tianmo-shengong",1)/2;
        ...
        if( (int)me->query_skill("tianmo-shengong",1) < 100 )
                return notify_fail("你的天魔神功功力不够，不能吸取对方的内力！n");
	if ( me->query_temp("sucked") )
    ...
    message_vision( HIR "$N凝神一聚，运起吸心大法，全身骨节发出一阵爆豆般的声响，似乎要杀人了。\n" NOR, me);
...
{
                target->add("qi",  -1*(1+(me->query_skill("tianmo-shengong", 1)-90)) );
                target->add("eff_qi",  -1*(1+(me->query_skill("tianmo-shengong", 1)-90)/5) );
        ...
        if( (int)target->query("qi") < (int)me->query("max_neili")/18 )
{
                ...
                target->add("qi",  -1*(1+(me->query_skill("tianmo-shengong", 1)-90)) );
                target->add("eff_qi",  -1*(1+(me->query_skill("tianmo-shengong", 1)-90)/6) );
                call_out("del_sucked", 10, me);
                me->add("max_neili",  1*(1+(me->query_skill("tianmo-shengong", 1)-90)/6) );
