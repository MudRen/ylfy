...
int perform(object me, object target)
{
	...
	if( (me->query("tianmo-zhang")<200)&&(me->query("tianmo-zhang")>100) )
	{
		if(target->is_busy())
			return notify_fail(target->name()+"已经手忙脚乱了！\n");
		message_vision(HIY"\n$N脚踩连环，如天魔狂舞，一时间，将$n包围在千百双掌影中！\n\n"+NOR,me,target);
		if(random((int)target->query("xyzx_sys/level")/2) < (int)me->query("xyzx_sys/level"))
		{
			message_vision(HIR"只见"+target->query("name")+"四周就象有千百只"HIY"魔"HIC"影"HIM"在"HIG"飞"HIB"舞"HIY",\n
			               结果$N被扰得眼花潦乱，接应不暇！手脚顿时慢了下来..\n"NOR,target);
			target->start_busy(5);
	...
	if( me->query("tianmo-zhang")>200 )
	{
		if(target->is_busy())
			return notify_fail(target->name()+"已经手忙脚乱了！\n");
		message_vision(HIY"\n$N脚踩连环，如天魔狂舞，一时间，将$n包围在千百双掌影中！\n\n"+NOR,me,target);
		if(random((int)target->query("xyzx_sys/level")) < (int)me->query("xyzx_sys/level"))
