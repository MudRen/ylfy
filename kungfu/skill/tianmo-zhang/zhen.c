...
int perform(object me, object target)
{
        ...
        if ((int)me->query_skill("tianmo-zhang", 1) < 520)
                return notify_fail("天魔掌法火候不够，难以施展" ZHEN "。\n");

        if (me->query_skill_mapped("unarmed") != "tianmo-zhang")
        ...
        dp = (int)target->query("xyzx_sys/level");

        if (ap / 2 + random(ap) > dp)
        { 
                damage = me->query_skill("tianmo-zhang",1) + random(me->query_skill("unarmed"))+ 500;
        ...
        {
                msg += CYN "$n" CYN "见$N" CYN "这掌来势非凡，不敢轻易招架，当即飞身纵跃闪开。\n" NOR;
        }
