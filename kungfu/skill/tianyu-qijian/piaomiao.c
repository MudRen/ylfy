...
int perform(object me, object target)
{
    ...
    if( !wizardp(me) &&(int)me->query_skill("tianyu-qijian", 1) < 1500 )
		return notify_fail("你的天羽奇剑等级不够!不能贯通使用！\n");
    
	if( !wizardp(me) &&(int)me->query_skill("liuyang-zhang", 1) < 1100 )
		return notify_fail("你的天山六阳掌等级不够!不能贯通使用！\n");
	
	if(!wizardp(me) && (int)me->query_skill("bahuang-gong", 1) < 1000 )
    ...
    if ( !wizardp(me) && me->query_skill_mapped("force") != "bahuang-gong")
	   ...
	   me->add("neili", -2000);
        ...
        i = me->query_skill("tianyu-qijian", 1) + me->query_skill("bahuang-gong", 1);
		damage = me->query_skill("tianyu-qijian") + me->query_skill("sword", 1) + me->query_skill("bahuang-gong") * me->query("con");
        j = me->query_skill("tianyu-qijian", 1) + me->query_skill("bahuang-gong", 1);
	...
	if (random(me->query("combat_exp")) > target->query("combat_exp")/3 ){
     ...
     weapon->unequip();
    if (random(1)==0) target->start_busy(3);
        msg = YEL "$N微微一笑，左手向$n轻轻弹出，好像是要弹去梅花上面的露珠！" NOR;
        COMBAT_D->do_attack(me,target, me->query_temp("weapon"), 1,msg);                                             
        message_vision(HIW"【弹 梅】"NOR, me, target); 
        target->receive_wound("qi",random( me->query_skill("liuyang-zhang",1)));
