...
mapping query_action(object me, object weapon)
{
    ...
    else if (random(me->query_skill("xianglong-zhang",1)) > 250 && me->query_skill("huntian-qigong",1) > 200 && me->query("neili") > 200)
        {
	  me->add("neili", -50);
        return ([
    ...
    else if (random(me->query_skill("xianglong-zhang",1)) > 200 && me->query_skill("huntian-qigong",1) > 100 && me->query("neili") > 200)
     {
	me->add("neili", -50);
        return ([
...
/*int ob_hit(object ob, object me, int damage)
{
	...
	{
		if(living(me) &&
		        random(me->query("xyzx_sys/level")) > ob->query("xyzx_sys/level")/3 &&
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        if( random( me->query_skill("unarmed") ) > random( victim->query_skill("force")) ) 
        {
                ...
                return random(2) ? HIR "只听$n" HIR "前胸「咔嚓」一声脆响，竟像是肋骨断折的声音。\n" NOR:

                                   HIR "$n" HIR "一声惨叫，胸前「咔嚓咔嚓」几声脆响，口中鲜血狂喷。\n" NOR;
        }
}
