...
int perform(object me, object target)
{
	...
	if( (int)me->query_skill("xuanhualiu-quanfa", 1) < 2200 )
	...
	if ((int)me->query_skill("shayi-xinfa", 1) < 1100)
	...
	if( (int)me->query_skill("unarmed", 1) < 170 )
		return notify_fail("你的拳法不够娴熟，使不出「三重劲」功的绝招！\n");

	if ((int)me->query_skill("shayi", 1) < 1280)
		return notify_fail("你的杀意火候不够。\n");

	msg = HIW "\n$N突然全身骨骼作响打出一拳，脸上一阵红色，全身好象有使不完的力量一样！\n   一拳打出，拳风直逼$n而去！\n"NOR;

	if (random(me->query_skill("unarmed")) > target->query_skill("dodge") /3 || random(me->query("xyzx_sys/level")) > target->query("xyzx_sys/level")/2)
	{
		me->start_busy(1);
		target->start_busy(1);
		me->add("neili", -200);
...
int perform2(object me, object target, int p)
{
	...
	if (random(me->query_skill("unarmed")) > target->query_skill("parry") / 3 && me->query_skill("shayi-xinfa", 1) > 199 )
	{
		me->start_busy(1);
		target->start_busy(1);
		me->add("neili", -200);
...
int perform3(object me, object target, int p)
{
	...
	if (random(me->query_skill("unarmed")) > target->query_skill("dodge") / 3 && me->query_skill("xuanhualiu-quanfa", 1) > 249 )
	{
		me->start_busy(2);
