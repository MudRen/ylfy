...
int perform(object me, object target)
{
	...
	if( me->query_skill("xuantie-jianfa") < 1000 )
	...
	ap=me->query("xyzx_sys/level") + me->query_skill("dodge",1) + me->query_skill("sword",1);
	dp=target->query("xyzx_sys/level") + target->query_skill("dodge",1) + target->query_skill("parry",1);
	damage=me->query_skill("force")+me->query_skill("sword");
	damage *=2;
	if ( ap/3 + random(ap*5/3) > dp/3 + random(dp*4/3))
	{
		msg += HIB"$N力道使得极猛,挺剑送出。$n早有提防，却万想不到来势竟是如此猛烈，眼见闪避不及，只听砰的一声猛响，登时连退了五六步，才勉强拿椿站定，等时脸如金纸。\n"NOR;
	...
	{
		msg += HIB"$N一剑击出，右手长袖顺势拂去，$n顷刻间只感五脏六腑都似翻转了，站在当地，既不敢运气，也不敢移动半步，便如僵了一般。\n"NOR;
		if (! target->is_busy())
