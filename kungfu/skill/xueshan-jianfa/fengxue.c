...
int perform(object me, object target)
{
	...
	if(random( me->query_skill("xueshan-jianfa",1)+me->query_skill("dodge",1)) > (target->query_skill("dodge",1)+target->query_skill("parry",1)))
	{
		msg +=HIG "$n只觉得四周压力越来越大，道道暗流缠绕周身，几乎动弹不得。\n" NOR;
		target->start_busy( (int)me->query_skill("xueshan-jianfa",1) / 50+2);
		me->add("neili", -100);
	}
	else
	{
