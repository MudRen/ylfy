...
int perform(object me)
{
        ...
        ap = random(me->query("xyzx_sys/level") + skill);
        ...
        COMBAT_D->report_status(target);
            qi_wound = (int)me->query_skill("xueshan-jianfa",1)+(int)me->query_skill("sword");
        ...
        if( random(me->query("xyzx_sys/level")) > target->query("xyzx_sys/level")/4 ) {
         attack_time = random((int)me->query_skill("xueshan-jianfa", 1) / 30);
