...
int perform(object me, object target)
{
	...
	if( me->query("xyzx_sys/level") > target->query("xyzx_sys/level") / 3 )
	{
		...
		if (! objectp(ob))
		{
			...
			if (target->query_skill("dodge",1) > 200 && random(2) == 0)
			{
				switch (random(2))
				{
				case 0:
			...
			else if (target->query_skill("force",1) > 200 && random(2) == 0)
			{
				switch(random(2))
				{
				case 0:
...
//					target->unconcious();
		...
		{
			msg += HIW " $p连忙招架，只听见「啪」地一声，$p手中的" + ob->name() + "寸寸断裂，散落一地！\n";
			ob->unequip();
