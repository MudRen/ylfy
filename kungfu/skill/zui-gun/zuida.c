...
int perform(object me, object target)
{
    ...
    cnt = (int)me->query_condition("drunk") / 3 ;
    
    if (cnt > 30)
        cnt = 30;
        
    count = me->query_str() + random(cnt * cnt);
    count1 = me->query_dex() + random(cnt * cnt);
    me->add_temp("sk_up/attack",  count);
    me->add_temp("sk_up/dodge", count1);
    me->set_temp("zuida", 1);
...
void remove_effect(object me, int amount, int amount1)
{
    ...
    if ( (int)me->query_temp("zuida") ) 
    {
        me->add_temp("sk_up/attack",  -amount);
        me->add_temp("sk_up/dodge", -amount1);
